...
model_restored.cuda()
save_dir = 'F:\\imageDe\\SUNet-main\\visdom'
...
for i, tensor in enumerate(outs):
    for j in range(tensor.size(0)):  # 假设每个张量包含 4 个图像
        # 将张量的每个通道归一化到 [0, 1]
        if tensor[j].dtype != torch.float32:
            tensor[j] = tensor[j].float()
        if tensor[j].max() > 1:
            tensor[j] = tensor[j] / 255.0
        tensor[j] = tensor[j] * 255.0
        img_PIL = toPIL(tensor[j])  # 张量tensor转换为图片
        img_PIL.save(os.path.join(save_dir, f'image_{i}_{j}.png'))  # 保存图片；img_PIL.show()可以直接显示图片
